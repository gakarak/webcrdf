function DrugResistorScript(xrorig, xrsegm, ctorig, ctsegm, outdir)

if ~isempty(outdir) && (outdir(end) ~= '/' || outdir(end) ~= '\')
    outdir = [outdir '/'];
end
...
ver = 'v1.0';
logmess(outdir, '');
logmess(outdir, ['DrugResistorScript ' ver ' started']);
...
try
    useXray = controlInputs(xrorig, xrsegm, ctorig, ctsegm);
    
    if ~useXray
        logmess(outdir, 'Using X-ray - Disabled', 1);
    else
        logmess(outdir, 'Using X-ray - Enabled', 1);
        
        logmess(outdir, ['Reading X-ray image from "' xrorig '"'], 1);
        xr0 = double(dicomread(xrorig));
        logmess(outdir, ['Reading X-ray mask from "' xrsegm '"'], 1);
        if strcmpi(xrsegm(end - 3:end), '.dcm')
            xrsgm = dicomread(xrsegm) > 0;
        else
            xrsgm = imread(xrsegm) > 0;
        end
        
        writeprogress(outdir, 35);
        logmess(outdir, 'Describing X-ray image', 1);
        xrdesc = xraydescribe(outdir, xr0, xrsgm);
    end
    writeprogress(outdir, 45);
    
    logmess(outdir, ['Reading CT image from "' ctorig '"'], 1);
    s = load_nii(ctorig);
    ct0 = s.img;
    pixdim = s.hdr.dime.pixdim(2:4);
    pixdim = pixdim / pixdim(1);
    logmess(outdir, ['Reading CT mask from "' ctsegm '"'], 1);
    s = load_nii(ctsegm);
    ctsgm = s.img;
    writeprogress(outdir, 50);
    logmess(outdir, 'Describing CT image', 1);
    ctdesc = ctdescribe(outdir, ct0, pixdim, ctsgm);
    writeprogress(outdir, 90);
    
    logmess(outdir, 'Loading the trained classifiers', 1);
    rp = trainClassifiers(outdir, useXray);
    writeprogress(outdir, 95);
    
    if useXray
        [~, ~, ~, lprob] = rp.predict({ctdesc, xrdesc}, []);
    else
        [~, ~, ~, lprob] = rp.predict({ctdesc}, []);
    end
    logmess(outdir, 'Prediction Results', 1);
    str = sprintf('Probability of DR = %i%%', round(lprob * 100));
    logmess(outdir, str, 1);
    fid = fopen([outdir 'res.txt'], 'wt');
    fprintf(fid, 'Probability of Drug Resistance: %i%%\n', ...
        round(lprob * 100));
    fclose('all');
    writeprogress(outdir, 100);
catch err
    logmess(outdir, ['ERROR: ' err.message], 1);
    for i = 1:length(err.stack)
        st = err.stack(i);
        logmess(outdir, ['(' num2str(i) ') ' st.file ...
            ' -- line ' num2str(st.line)], 1);
    end
    fid = fopen([outdir 'err.txt'], 'wt');
    fprintf(fid, '%s\n', err.message);
    fclose('all');
end
...
function useXray = controlInputs(xrorig, xrsegm, ctorig, ctsegm)
...
function writeprogress(outdir, i)

dlmwrite([outdir 'progress.txt'], i);
disp(num2str(i));

function logmess(outdir, str, onScreen)

if nargin < 3
    onScreen = false;
end

logfile = [outdir 'DrugResistor.log'];
...
function desc = xraydescribe(outdir, xray, msk)

% logmess(outdir, 'DEBUG: RANDOM XRAY DESCRIPTOR', 1);
% desc = rand(1, 1536);
% return;
...
im = xray;
qc = quantile(im(msk(:)), 0.5);
im = im - qc + 5000;
mnmx = [4534 5593];
m = (im - mnmx(1)) / (mnmx(2) - mnmx(1));
m(m < 0) = 0;
m(m > 1) = 1; 

m(~msk) = nan;

desc = cooccur2D(outdir, m, iRange, iBins, dists, nDots);
desc = desc / sum(desc(:));

function desc = cooccur2D(outdir, m, iRange, iBins, dists, nDots)
...
for id = 1:length(dists)
    ...
    writeprogress(outdir, round(progr));
...
function desc = ctdescribe(outdir, ct, pixdim, sgm)

m = single(ct) + 1024;
m(sgm < -3000) = nan;
m = m / 1500;

desc = superdescribe(outdir, m, 16, 0.3, []);
desc = desc / sum(desc(:));

function desc = cooccur3D(outdir, m, iRange, iBins, gRange, gBins, aBins, dists, pixdim)
...
function rp = trainClassifiers(outdir, useXray)

sss = importdata('training/training_data_info.txt');
ct = dlmread('training/training_data_ct.txt');
xr = dlmread('training/training_data_xray.txt');
dr = dlmread('training/training_data_dr.txt');
nm = dlmread('training/training_data_nmax.txt');

logmess(outdir, sss{1});

if useXray
    rp = ResistPredictor.fit({ct, xr}, [], dr, nm);
else
    rp = ResistPredictor.fit({ct}, [], dr, nm);
end
...
function [fhists, chist, ccm, spFeatures, ftMaps] = superdescribe(outdir, im3, sz, reg, resz, dct, msk3)
%v3.4
